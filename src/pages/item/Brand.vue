...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      deleteBrand(brand){ // 新增：删除品牌提示
        this.$message.confirm('此操作将永久删除数据，是否继续?', '提示', {
          confirmButtonText: '确定删除',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
            this.handleDelete(brand.id);
        }).catch(()=>{
          this.$message.info('已取消删除');
        })
      },
      handleDelete(bid){// 新增：删除品牌
        this.$http.delete("/item/brand/"+bid).then(res => {
          this.brands = this.brands.filter( brand => brand.id != bid)
