  ...
  export default {
    ...
    data: function () {
      return {
        ...
        open:false,
        beginEdit:false,
        childCategoryList:[],// 新增，用来在删除时判断是否有子列表
      }
    },
    watch:{
      parentState(val){
    ...
    methods: {
      ...
      addChild: function () {
        ...
        if (!this.model.isParent) {
          ...
          this.model.isParent = true; // 新增：修改为是父亲的节点应该将修改保存到数据库
          this.updateParentIsParent(this.model.id);
          this.open = true;
      ...
      deleteChild: function () {
        this.$message.confirm('此操作将永久删除数据，是否继续?', '提示', {
          ...
          if (!this.ifHaveChildCategory(this.model.id)) { // 新增： 如果该类别下还有类别，将不能删除
              alert("该类别下还有子类别，请删除所有的子类别后再试!")
          }else {
      ...
      updateParentIsParent(id) { // 新增
        this.$http.put("/item/category/isParent/"+id)
      },
      ifHaveChildCategory(id) { // 新增
        this.$http.get(this.url, {params: {pid: id}}).then(res => {
          this.childCategoryList = res.data
          console.log("aaaaaaaaa" + res)
        })
        console.log(this.childCategoryList)
        console.log(this.childCategoryList.length === 0)
        return this.childCategoryList.length === 0;
      }
