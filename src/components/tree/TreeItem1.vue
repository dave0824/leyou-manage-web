  ...
  export default {
    ...
    data: function () {
      return {
        ...
        open:false,
        beginEdit:false,
        child: {}, // 新增，将child放在这，便于获取新的自增长id
      }
    },
    watch:{
      parentState(val){
    ...
    methods: {
      ...
      addChild: (event)=> { // 将函数改为箭头函数，防止this指向错误
         this.child = {
          id: 0,
          name: '新的节点',
          parentId: event.target.id,
          isParent: false,
          sort:event.target.children? event.target.children.length + 1:1
        }
        if (!event.target.isParent) {
          this.handleAdd(this.child); // 先添加到数据库再回写id后再添加到model
          Vue.set(event.target, 'children', [this.child]);
          event.target.isParent = true;
          this.open = true;
        } else {
          if (!this.isFolder) {
            this.$http.get(this.url, {params: {pid: this.model.id}}).then(resp => {
              this.handleAdd(this.child);
              Vue.set(event.target, 'children', resp.data);
              this.model.children.push(this.child);
              this.open = true;
            });
          } else {
            this.handleAdd(this.child);
